...
/**
 * SCROLL-DRIVEN 360° HOUSE — sticky canvas, scroll rotates frame by frame.
 */
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const idx = Math.min(
        ...
        Math.floor(progress * rotationFrames.length)
      );
      setFrameIdx(idx);
    };
    onScroll();
        ...
        <div className="relative z-10 mx-auto flex h-full max-w-[1600px] w-full flex-col justify-between px-6 py-10 md:px-10 md:py-16">
          ...
          <div className="flex flex-col items-center gap-6 self-center">
              ...
              {rotationFrames.map((_, i) => (
                <span
                  ...
                  className={`h-px transition-all duration-300 ${
                    i === frameIdx ? "w-10 bg-gold" : "w-5 bg-cream/30"
                  }`}
                />
              ))}
            </div>
            <p className="label text-cream/70">
              ...
              {String(rotationFrames.length).padStart(2, "0")}
            </p>
          </div>
