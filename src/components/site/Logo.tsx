export function Logo({ compact = false }: { compact?: boolean }) {
  return (
    <span className="flex items-center gap-3 select-none">
      ...
      >
        <span className="font-display text-[15px] font-light tracking-[0.15em] leading-none">
          TS
        </span>
      ...
      {!compact && (
        <span className="flex flex-col leading-tight">
          <span className="font-display text-[15px] font-light tracking-[0.32em] text-espresso uppercase">
            Terra
          </span>
          <span className="label text-[10px] tracking-[0.4em] text-caramel">
            Space Studio
          </span>
        </span>
      )}
